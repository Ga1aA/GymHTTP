...
// curl => transfiere un URL, es una herramienta para transferir hacia/desde un servidor
// utilizando uno de los protocolos soportados, este comando esta diseñado para trabajar
//sin la interacción con el usuario
...
public:
	...
	ClientReal()
	{
		CURL* c = curl_easy_init();// empieza una libcurl sesion fácil
		// curl_easy_setopt te pide: CURL* handle, CURLoption, parameter
		curl_easy_setopt(c, CURLOPT_NOSIGNAL, 1);// 
		curl_easy_setopt(c, CURLOPT_CONNECTTIMEOUT_MS, 3000);
		curl_easy_setopt(c, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4);//Permite a una aplicacion seleccionar que tipo de direccion IP usar en la resolicion de un host name, en este caso es IPv4
		curl_easy_setopt(c, CURLOPT_FOLLOWLOCATION, true);// es un parametro seteado a 1, que dice a la libreria seguir cualquier Location (cabecera que el servidor envía como parte de una cabecera HTTP), esto significa que, la libreria reeviará el mismo request en la nueva location y la siguienre nueva location, hasta que no sean devueltas más cabeceras
		curl_easy_setopt(c, CURLOPT_SSL_VERIFYPEER, 0);// pasa un long como un parámetro. Determina si CURL verefica la autenticidad del del certificado del peer para indicar su identidad, como el valor es 0, no verifica
		curl_easy_setopt(c, CURLOPT_SSL_VERIFYHOST, 0);// Pasa un long como un parametro. Determina si llibcurl verifica que el servidor
